...
uint32_t CPU::GrabModRMAddress(std::string& disasm)
{
    if (!a32)
    {
        ...
        case 0x00:
            ...
            {
            case 0x06:
                disp16 = bus->read<uint16_t>(TranslateAddress(eip, CS));
                disasm = "[0x" + convert_int(TranslateAddress(disp16, prefix)) + "]";
                isDisp16 = true;
                eip += 2;
                return disp16;
            }
    ...
    else
    {
        ...
        case 1:
        {
            ...
            {
            case 6:
                disasm = "[esi + 0x" + convert_int(disp8) + "]";
...
void CPU::SetRM(std::string& disasm, uint32_t val)
{
	...
	printf("Write to memory unimplemented\n");
	exit(1);
}
